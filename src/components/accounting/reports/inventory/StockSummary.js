// src/components/accounting/reports/inventory/StockSummary.js
'use client';
...
const StockSummaryReport = ({ companyId }) => {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      switch (format) {
        case 'excel':
          await exportToExcel(exportData, 'stock-summary-report');
          success('Report exported to Excel successfully');
          break;
        case 'pdf':
          await exportToPDF(exportData, 'stock-summary-report');
          success('Report exported to PDF successfully');
          break;
        case 'json':
          await exportToJSON(exportData, 'stock-summary-report');
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold text-slate-800">{reportData.summary.total_stock.toLocaleString('en-IN')}</div>
              </div>
              <div className="bg-orange-50 p-4 rounded-lg">
                <div className="text-sm text-slate-600">Total Value</div>
                <div className="text-2xl font-bold text-slate-800">{formatCurrency(reportData.summary.total_value)}</div>
              </div>
            </div>
              ...
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Stock Value</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-slate-200">
                  {reportData.products.map((product) => (
                    <tr key={product.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                        {product.current_stock?.toLocaleString('en-IN') || 0}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                        {product.unit || 'N/A'}
                      </td>
                ...
                <tfoot className="bg-slate-50 font-semibold">
                  <tr>
                    ...
                    <td className="px-6 py-3 text-sm text-slate-900">{reportData.summary.total_stock.toLocaleString('en-IN')}</td>
                    <td className="px-6 py-3 text-sm text-slate-900"></td>
                    <td className="px-6 py-3 text-sm text-slate-900"></td>
                    <td className="px-6 py-3 text-sm text-slate-900">{formatCurrency(reportData.summary.total_value)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>

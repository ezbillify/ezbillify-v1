// src/components/accounting/reports/sales/CustomerWiseSales.js
'use client';
...
const CustomerWiseSalesReport = ({ companyId }) => {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      switch (format) {
        case 'excel':
          await exportToExcel(exportData, 'customer-wise-sales-report');
          success('Report exported to Excel successfully');
          break;
        case 'pdf':
          await exportToPDF(exportData, 'customer-wise-sales-report');
          success('Report exported to PDF successfully');
          break;
        case 'json':
          await exportToJSON(exportData, 'customer-wise-sales-report');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Customer-wise Sales Report</h1>
              ...
              <table className="min-w-full divide-y divide-slate-200">
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {reportData.customers.map((customer, index) => (
                    <tr key={customer.customer_id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                        {formatCurrency(customer.tax_amount)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <span className={customer.grand_total >= 0 ? 'text-green-600' : 'text-red-600'}>
                ...
                <tfoot className="bg-slate-50 font-semibold">
                  <tr>
                    ...
                    <td className="px-6 py-3 text-sm text-slate-900">{reportData.customers.reduce((sum, customer) => sum + customer.invoices.length, 0)}</td>

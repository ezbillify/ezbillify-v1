// src/components/accounting/reports/purchase/SupplierWisePurchase.js
'use client';
...
const SupplierWisePurchaseReport = ({ companyId }) => {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      switch (format) {
        case 'excel':
          await exportToExcel(exportData, 'supplier-wise-purchase-report');
          success('Report exported to Excel successfully');
          break;
        case 'pdf':
          await exportToPDF(exportData, 'supplier-wise-purchase-report');
          success('Report exported to PDF successfully');
          break;
        case 'json':
          await exportToJSON(exportData, 'supplier-wise-purchase-report');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Supplier-wise Purchase Report</h1>
              ...
              <table className="min-w-full divide-y divide-slate-200">
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {reportData.suppliers.map((supplier, index) => (
                    <tr key={supplier.supplier_id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                        {formatCurrency(supplier.tax_amount)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <span className={supplier.grand_total >= 0 ? 'text-green-600' : 'text-red-600'}>
                ...
                <tfoot className="bg-slate-50 font-semibold">
                  <tr>
                    ...
                    <td className="px-6 py-3 text-sm text-slate-900">{reportData.suppliers.reduce((sum, supplier) => sum + supplier.invoices.length, 0)}</td>

          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-200">
              {salesOrder.items && salesOrder.items.map((item, index) => {
                // Calculate rate with tax included (total per unit)
                const rateWithTax = item.total_amount / item.quantity;
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-900">{formatCurrency(rateWithTax)}</td>

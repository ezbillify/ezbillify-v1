...
const SalesOrderForm = ({ salesOrderId, companyId, quotationId }) => {
  ...
  useEffect(() => {
    ...
    const initializeForm = async () => {
      // Reset initialization ref when creating a new sales order
      if (!salesOrderId) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        // Reset initialization ref to allow proper re-initialization for next new sales order
        initializationRef.current = false;
  ...
  const handleScanSuccess = (result) => {
    ...
    // Here you would typically add the scanned item to the sales order

// src/components/sales/QuotationForm.js
'use client';
...
const QuotationForm = ({ quotationId, companyId }) => {
  ...
  useEffect(() => {
    ...
    const initializeForm = async () => {
      ...
      console.log('🚀 Starting quotation form initialization...');
      ...
      try {
        ...
        if (quotationId && isMounted) {
          console.log('📝 Loading existing quotation...');
        ...
        if (isMounted && !quotationId) {
          setQuotationNumber('QT-0001/25-26');
        }
      }
    };
  ...
  useEffect(() => {
    ...
    console.log('📋 Customers fetched:', fetchedCustomers?.length || 0, fetchedCustomers);
  ...
  useEffect(() => {
    if (!quotationId && selectedBranch?.id && initializationRef.current && companyId) {
      console.log('🏢 Branch changed, updating quotation number preview...');
  ...
  const fetchNextQuotationNumber = async () => {
    ...
    if (selectedBranch?.id) {
      ...
      console.log('🏢 Using branch for quotation number:', selectedBranch.name || selectedBranch.branch_name);
    }
    ...
    if (result.success && result.data?.preview) {
      console.log('✅ Setting quotation number to:', result.data.preview);
  ...
  useEffect(() => {
    console.log('🔎 Customer Search Debug:', {
      ...
      sampleCustomers: customers.slice(0, 3).map(c => ({ name: c?.name, code: c?.customer_code, type: c?.customer_type }))
    });
  }, [customers, customerSearch, filteredCustomers.length, showCustomerDropdown]);
  ...
  const handleSubmit = async (e) => {
    ...
    const apiCall = async () => {
      ...
      if (!quotationId && selectedBranch?.id) {
        ...
        console.log('🏢 Saving quotation with branch:', selectedBranch.name || selectedBranch.branch_name);
      }
    ...
    if (result.success) {
      ...
      console.error('Quotation save failed:', result);
      showError(result.error || result.message || 'Failed to save quotation. Please try again.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
                    ...
                    {quotationId && quotationBranch ? (
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        <span className="text-sm font-medium text-blue-900">
                          Branch: {quotationBranch.name || quotationBranch.branch_name}
                        </span>
                      </div>
      ...
      <div className="p-4 space-y-4">
        {/* Top Row - Customer, Quotation Details, Summary */}
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-4 h-full flex flex-col">
              ...
              <div className="relative customer-input mb-3 flex-shrink-0" ref={customerDropdownRef}>
                      ...
                      <div className="p-4 text-center text-gray-500 text-sm">
                        {customers.length === 0 ? (
                          <div>
                            ...
                            <p className="text-xs mt-1">Please create a customer first</p>
                          </div>
                        ) : (
                          <div>

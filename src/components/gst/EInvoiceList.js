// src/components/gst/EInvoiceList.js
'use client';
...
const EInvoiceList = ({ companyId }) => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const result = await executeRequest(async () => {
        return await authenticatedFetch(`/api/gst/e-invoices?${params}`);
      });
  ...
  const handleGenerateEInvoice = async (invoice) => {
    try {
      const result = await executeRequest(async () => {
        return await authenticatedFetch('/api/gst/e-invoices', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            // In a real implementation, you would pass the actual invoice data
            invoice_data: {
      ...
      showError('Error generating e-Invoice: ' + (err.message || 'Unknown error'));
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your e-Invoice generation and status</p>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-yellow-700">
                <p>Connect your GST account to generate e-Invoices. You can view existing e-Invoices below.</p>
              </div>
            </div>
          </div>
        </div>
                ...
                invoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {invoice.status === 'pending' && credentials ? (
                        <Button
                          size="sm"
